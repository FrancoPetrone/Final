...
@section('content')
	<div class="container">
		<div class="row contenedor-panel">
			...
			<form class="forms" action="{{ route('panel.productos.edit', $producto->id) }}" method="POST" enctype="multipart/form-data">
				<input type="hidden" name="_token" value="{{ csrf_token()  }}">
				<input type="hidden" name="_method" value="PUT">
				<div class="form-group col-xs-12 col-sm-12 col-md-6 col-lg-6">
					<label><span class="glyphicon glyphicon-triangle-right"></span> Nombre</label>
					<input type="text" name="nombre" class="form-control" id="nombre" value="{{ $producto->nombre }}" placeholder="Nombre del Producto" pattern=".{5,20}" required/>
				</div>
				<div class="form-group col-xs-12 col-sm-12 col-md-6 col-lg-6">
					<label><span class="glyphicon glyphicon-info-sign"></span> Descripción</label>
					<input type="text" name="descripcion" class="form-control" id="descripcion" value="{{ $producto->descripcion }}" placeholder="Descripción del Producto" pattern=".{5,20}" required/>
				</div>
				<div class="form-group col-xs-12 col-sm-12 col-md-6 col-lg-6">
				...
				<div class="form-group col-xs-12 col-sm-12 col-md-6 col-lg-6">
					...
					<input type="text" name="precio" class="form-control" id="precio" value="{{ $producto->precio }}" placeholder="Precio del Producto" pattern=".{1,6}" required/>
				</div>
				<div class="form-group col-xs-12 col-sm-12 col-md-6 col-lg-6">
					<label><span class="glyphicon glyphicon-equalizer"></span> Stock</label>
					<input type="text" name="stock" class="form-control" id="stock" value="{{ $producto->stock }}" placeholder="Stock Disponible" pattern=".{1,5}" required/>
				</div>
				<div class="form-group col-xs-12 col-sm-12 col-md-6 col-lg-6">
					<label><span class="glyphicon glyphicon-picture"></span> Imágen (Opcional)</label>
					<input class="btn btn-default image-preview-input form-control" type="file" accept="image/*" name="imagen"/>
				</div>
